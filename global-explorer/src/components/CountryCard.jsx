...
const CountryCard = ({ country }) => {
  ...
  const isFavorite = useSelector(state => state.favorites[country.name.common]);
  ...
  return (
    <Link to={`/country/${country.name.common}`} 
          className="block bg-white shadow-lg rounded-lg overflow-hidden transition-shadow hover:shadow-xl">
      <div className="h-40 bg-gray-100 flex items-center justify-center">
        <img src={country.flags.svg} alt={`${country.name.common} flag`} className="w-full h-full object-cover" />
      </div>
      <div className="p-4">
        ...
        <p>Capital: <span className="font-medium">{country.capital?.[0] || 'N/A'}</span></p>
